...
def test_desktop_preview_is_a_truthful_webview2_shell() -> None:
    ...
    assert 'internal const string ProductVersion = "0.3.20"' in app
    assert '$ProductVersion = "0.3.20"' in build
    assert '$ProductVersion = "0.3.20"' in installer
...
def test_desktop_tabs_popups_and_modern_controls_are_wired() -> None:
    ...
    assert "PaintTabStrip(graphics, true)" in app
    assert "graphics.FillRectangle(background, 0, 0, ClientSize.Width, height)" in app

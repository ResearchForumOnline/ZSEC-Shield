...
def test_desktop_preview_is_a_truthful_webview2_shell() -> None:
    ...
    assert 'internal const string ProductVersion = "0.3.12"' in app
    assert '$ProductVersion = "0.3.12"' in build
    assert '$ProductVersion = "0.3.12"' in installer
...
def test_community_release_is_deterministic_and_publishes_provenance(
    ...
    name = "zsec-browser-community-0.3.12-windows-x64-unsigned.zip"
    ...
    with zipfile.ZipFile(archive_a) as archive:
        provenance = json.loads(
            archive.read(
                "zsec-browser-community-0.3.12/release-provenance.json"
            ).decode("utf-8")
        )

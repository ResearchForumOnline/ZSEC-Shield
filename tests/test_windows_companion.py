...
class WindowsCompanionStaticTests(unittest.TestCase):
    ...
    def test_launcher_uses_existing_watch_engine_and_quarantine_is_explicit(self) -> None:
        ...
        self.assertIn("function Invoke-ApplicationUpdateNoticeCheck", content)
        self.assertIn('"update-application-notice"', content)
        self.assertIn('"zsec.shield.application-update-status.v1"', content)
        self.assertIn("[bool]$result.automatic_install", content)
        self.assertIn("never downloads or", content)

...
COPY_REQUIREMENTS = {
    "zero-security": (
        "ZSEC Antivirus Community 0.3.11",
...
DOWNLOAD_PAGES: dict[str, DownloadPolicy] = {
    "zero-security/download": {
        ...
        "artifacts": {
            ...
            "/downloads/zero-security/zsec-antivirus-desktop-0.3.7-windows-x86_64.zip",
    ...
    "zero-browser/download": {
        ...
        "fingerprints": 2,
        "artifacts": {
            "/downloads/zero-browser/zsec-browser-community-0.3.7-windows-x64-unsigned.zip",
...
def validate_page(slug: str, canonical: str) -> None:
    ...
    for required_copy in COPY_REQUIREMENTS[slug]:
        require(required_copy in text, f"{slug}: required 0.3.11 claim boundary missing")
...
def validate_privacy_page() -> None:
    ...
    for required_copy in (
        ...
        require(required_copy in text, "zero-browser/privacy: 0.3.11 privacy boundary")
    htaccess = (page.parent / ".htaccess").read_text(encoding="utf-8")
